...
public class PostLikeController {
    ...
    @Operation(summary = "게시글 TOP5 조회 API", description = "게시글 TOP5를 조회 합니다")
    @ApiResponses({
            @ApiResponse(responseCode = "201", description = "CREATED: 좋아요 TOP5 조회 성공"),
            @ApiResponse(responseCode = "400", description = "BAD REQUEST: 좋아요 TOP5 조회 실패. 요청값 확인 필요합니다"),
            @ApiResponse(responseCode = "401", description = "Unauthorized: 인증 실패. 주로 JWT 에러"),
            @ApiResponse(responseCode = "403", description = "Forbidden: 권한이 없는 페이지. 주로 잘못된 URL"),
            @ApiResponse(responseCode = "500", description = "INTERNAL SERVER ERROR : 서버 다운 또는 로딩중"),
    })
    @GetMapping("/top5")
    public ResponseEntity<List<ResponsePostRecord>> getTop5PostLike() {
        List<ResponsePostRecord> resultList = postLikeService.getTop5PostLike();
