...
public class ScriptController {
    ...
    @PostMapping("/project")
    public ResponseEntity<?> createProject(@RequestBody RequestNewProject requestNewProject,
                                                @RequestHeader("Authorization")String auth) {
        if ((scriptService.createProject(requestNewProject,auth).getStatusCode().equals(HttpStatus.FORBIDDEN))){
            return ResponseEntity.status(HttpStatus.CONFLICT).body("중복된 프로젝트명");
        }
        return scriptService.createProject(requestNewProject,auth);
    }
    ...
    @GetMapping("/projects")
    public ResponseEntity<?> getAllProjects(@RequestHeader("Authorization")String auth) {
        Iterable<ResponseProjects> response = projectServiceClient.getAllProjectsByClient(auth);
        if (response == null){
